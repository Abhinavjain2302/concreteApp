...
connection.connect(function(err){
    ...
    //var sql = "CREATE TABLE IF NOT EXISTS CitySchema (cityName VARCHAR(255))";
...
router.post('/login', function(req, res, next){
    ...
    connection.connect(function(err){
    if(err) throw err;
    var sql = "select email,password from user where email='"+username+"'";
   ...
   if(err) throw err;
            ...
            User.comparePassword(password, result[0].password, function (err, isMatch) {
                if(err){
                ...
                jwt.sign({id: result[0].email}, secret, function(err, token){
                    if(err)handleError(err, null, res);
                    return getAllUserDashboardDetails(req, res, result[0].email,token);
                })
            });
...
router.post('/changepass', function(req, res){
   ...
   if(err) throw err;
			...
			bcrypt.compare(oldpass, result[0], function(err, match) {
				if(!match){
					res.json({
						success:false,
...
router.get('/getcities', function(req, res){
    ...
    connection.query("select * from CitySchema",function(err,result,fields){
   if(err) throw err;
   console.log("Query updated");
   res.json({
      success:true,
